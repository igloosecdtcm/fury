...
generate_must_sub_query <- function(str){
  ...
  } else if (str$operator == "range") {
    range_num <- strsplit(as.character(str[3]), fixed=TRUE, split= "_")[[1]]
    return(paste0('{\"range\" : { \"',str[1],'" : { \"gte\" : ', as.numeric(range_num[1]), '000', ', \"lte\" : ', as.numeric(range_num[2]), '000','}}}'))
  } else {
    return ()
  }
}
...
aggregation_insert <- function(job_id, aggregation_time, field_set, d){
  idx = 1
  aggregation_body <- list("job_id" = job_id, "aggregation_time"= aggregation_time)
  return(reqursive_find_aggregation_info(d, 1, field_set, aggregation_body, idx))
}

aggregation_empty_insert <- function(job_id, aggregation_time, field_set) {
  aggregation_body <- list(
    "job_id" = job_id, 
    "aggregation_time"= aggregation_time
    )
  for (field_idx in 1 : length(field_set$field_name)) {
    aggregation_body[[paste0(field_set$field_name[[field_idx]], "_empty")]] <- paste0(field_set$field_name[[field_idx]])
    aggregation_body[[paste0(field_set$field_name[[field_idx]], "_count")]] <- 0
  }
  
  tryCatch({
    docs_create(index=fury_config$aggregationIndex, type='_doc', id=gsub("\\.", "", paste0(as.numeric(Sys.time()))), body=aggregation_body)
    return(1)
  },
  error = function(e) {
    print(paste0(aggregation_body, " - [", e, "]"))
    return(0)
  },
...
reqursive_find_aggregation_info <- function(d, field_idx, field_set, aggregation_body, idx){
  if (field_idx == 1) {
    for (key_idx in 1 : length(d$key)) {
      ...
      if (length(field_set$field_name) == 1) {
        ...
        reqursive_find_aggregation_info(d[[paste0(field_set$aggs_name[[field_idx+1]], ".buckets")]][[key_idx]], field_idx+1, field_set, aggregation_body, idx)
      }
    }
  } else {
